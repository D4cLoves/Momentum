...
export function RecoverCodePage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      navigate(`/recover-code/verify?email=${encodeURIComponent(email.trim())}`)
    } catch (submitError) {
      const message =
        submitError instanceof Error ? submitError.message : "Failed to send recovery code"
      setError(message)
    } finally {
      setLoading(false)
    }
            ...
            <div className="flex min-h-0 flex-col justify-center pt-8">
              <div className="max-w-3xl">
                ...
                <p className="mt-5 max-w-xl text-sm leading-7 text-muted-foreground sm:text-base">
                  Type the email linked to your account. We will send a
          ...
          <article className={`${framePanel} flex min-h-0 flex-col p-6 sm:p-8`}>
            <div className="flex items-start justify-between gap-4 border-b border-border/60 pb-5">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold">Email address</h2>
              </div>
            ...
            <form className="mt-6 flex min-h-0 flex-1 flex-col" onSubmit={handleSubmit}>
              ...
              <div className="mt-auto space-y-4 pt-6">
                <div className={`${softCard} flex items-start gap-3 px-4 py-4 text-sm text-muted-foreground`}>
                  ...
                  <p>
                    We will send a one-time code to this email. Use it on the next
                    step to continue password reset.
                  </p>
                </div>
